...
function pendingEmailHtml(d: { forApeg: boolean; eventName: string; eventDate: string; club: string; players: { name: string; email?: string; phone?: string; doc?: string; pkg: string | null; amount: number }[]; total: number; payUrl: string }) {
  const rows = d.players.map((p) => `<tr>
    <td style="padding:12px 14px;border-bottom:1px solid #e4ede2;color:#1f2e22;font-size:14px;font-weight:600;">${p.name}${d.forApeg && p.doc ? `<br><span style="color:#7a8a7a;font-size:12px;font-weight:400;">CC ${p.doc}${p.email ? ' · ' + p.email : ''}${p.phone ? ' · ' + p.phone : ''}</span>` : ''}</td>
    ...
    <td style="padding:12px 14px;border-bottom:1px solid #e4ede2;color:#0e2f1f;font-size:14px;font-weight:800;text-align:right;white-space:nowrap;">${_cop(p.amount)}</td>
  </tr>`).join('')
  ...
  <div style="max-width:560px;margin:0 auto;padding:28px 14px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
    <div style="background:#ffffff;border:1px solid #dfe7df;border-radius:22px;overflow:hidden;">
      <div style="padding:30px 30px 18px;text-align:center;">
        ...
        <h1 style="margin:20px 0 6px;color:#0e2f1f;font-size:23px;font-weight:900;letter-spacing:-0.5px;">${d.forApeg ? 'Nueva inscripción — falta pago' : 'Ya casi… completa tu pago'}</h1>
        <p style="margin:0;color:#5a6a5a;font-size:14px;line-height:1.6;">${d.forApeg ? 'Se registró una inscripción que aún no ha pagado.' : 'Tu inscripción quedó registrada. Para asegurar tu cupo solo falta completar el pago.'}</p>
      </div>
      <div style="padding:0 22px 8px;"><div style="background:#f4f8f3;border:1px solid #e4ede2;border-radius:16px;padding:16px 18px;">
        ...
        <table style="width:100%;"><tr><td style="color:#5a6a5a;font-size:13px;">${d.eventDate || 'Fecha por confirmar'}</td><td style="color:#5a6a5a;font-size:13px;text-align:right;">${d.club || ''}</td></tr></table>
      </div></div>
      <div style="padding:14px 22px 8px;"><table style="width:100%;border-collapse:collapse;border:1px solid #e4ede2;border-radius:16px;overflow:hidden;">
        <thead><tr style="background:#f0f5ef;"><th style="padding:11px 14px;text-align:left;color:#6b7c6b;font-size:11px;text-transform:uppercase;">Participante</th><th style="padding:11px 14px;text-align:left;color:#6b7c6b;font-size:11px;text-transform:uppercase;">Plan</th><th style="padding:11px 14px;text-align:right;color:#6b7c6b;font-size:11px;text-transform:uppercase;">Valor</th></tr></thead>
        <tbody>${rows}</tbody></table></div>
      <div style="padding:12px 22px 8px;"><div style="background:#f2f9e9;border:1px solid #d7e9b6;border-radius:16px;padding:15px 18px;">
        <table style="width:100%;border-collapse:collapse;"><tr>
          <td style="color:#6b7c6b;font-size:12px;font-weight:800;letter-spacing:1.5px;text-transform:uppercase;vertical-align:middle;">Total a pagar</td>
          <td style="color:#2f6d10;font-size:21px;font-weight:900;text-align:right;vertical-align:middle;white-space:nowrap;">${_cop(d.total)}</td>
        </tr></table>
      </div></div>
      ${d.forApeg ? '' : `<div style="text-align:center;padding:6px 22px 22px;"><a href="${d.payUrl}" style="display:inline-block;background:#0e2f1f;color:#a3e635;font-weight:900;font-size:15px;text-decoration:none;padding:15px 36px;border-radius:999px;">Completar el pago</a></div>`}
      <div style="padding:18px 30px 28px;border-top:1px solid #e4ede2;text-align:center;"><p style="margin:0;color:#8a9a8a;font-size:12px;line-height:1.7;">¿Dudas? <a href="mailto:${GMAIL_USER}" style="color:#2f6d10;text-decoration:none;font-weight:700;">${GMAIL_USER}</a><br>APEG &bull; Amor por el Golf</p></div>
    </div>
  </div></body></html>`
}
